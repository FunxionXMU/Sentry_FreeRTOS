...
public:
	...
	uint16_t robotId = UI_Data_RobotID_RStandard2;
	uint16_t clientId = UI_Data_CilentID_RStandard2;
	...
	struct {
		...
		struct {
			...
			uint8_t power_management_gimbal_output : 1;//gimbal口是否有输出
			uint8_t power_management_chassis_output : 1;//chassis口是否有输出
			uint8_t power_management_shooter_output : 1;//shooter口是否有输出
		}robot_status_t;
...
private:
	...
	graphic_data_struct_t graphicData;
	string_data_struct_t bulletDataStatic;
	string_data_struct_t powerDataStatic;
	float_data_struct_t bulletDataDynamic;
};
